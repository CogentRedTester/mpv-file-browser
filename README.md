...
If `user-input.lua` is loaded by mpv, and `user-input-module` is in the `~~/scripts/` directory, then using `Alt+o` will open an input box that can be used to directly enter directories for file-browser to open.
